...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("BZMMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("BZMMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("BZMMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("blazarium-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("BZMMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running BZMMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadBZMMiner() exception");
    } catch (...) {
        LogPrintf("ThreadBZMMiner() exception");
    }

    LogPrintf("ThreadBZMMiner exiting\n");
}
