...
class TxViewDelegate2 : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate2() : QAbstractItemDelegate(), unit(BitcoinUnits::BZM)
    {
    }
...
void Coinmix::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("BZM")
    //updateDisplayUnit();
}
...
void Coinmix::updateDarksendProgress()
{
    ...
    QString strAnonymizeBZMAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeBZMAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeBZMAmount = strAnonymizeBZMAmount.remove(strAnonymizeBZMAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBZMAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeBZMAmount * COIN) nMaxToAnonymize = nAnonymizeBZMAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeBZMAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeBZMAmount));
        strAnonymizeBZMAmount = strAnonymizeBZMAmount.remove(strAnonymizeBZMAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBZMAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
