...
Value mnbudget(const Array& params, bool fHelp)
{
    ...
    if (fHelp ||
        ...
        throw runtime_error(
            ...
            "  vote-many          - Vote on a BZM initiative\n"
            "  vote-alias         - Vote on a BZM initiative\n"
            "  vote               - Vote on a BZM initiative/budget\n"
    ...
    if (strCommand == "prepare") {
        ...
        if (params.size() != 7)
            throw runtime_error("Correct usage is 'mnbudget prepare proposal-name url payment_count block_start blazarium_address monthly_payment_blazarium'");
    ...
    if (strCommand == "submit") {
        ...
        if (params.size() != 8)
            throw runtime_error("Correct usage is 'mnbudget submit proposal-name url payment_count block_start blazarium_address monthly_payment_blazarium fee_tx'");
