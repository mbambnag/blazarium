...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "\nStop Blazarium server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "Blazarium server stopping";
}
...
static const CRPCCommand vRPCCommands[] =
    {
        ...
        /* BZM features */
        {"blazarium", "masternode", &masternode, true, true, false},
        {"blazarium", "masternodelist", &masternodelist, true, true, false},
        {"blazarium", "mnbudget", &mnbudget, true, true, false},
        {"blazarium", "mnbudgetvoteraw", &mnbudgetvoteraw, true, true, false},
        {"blazarium", "mnfinalbudget", &mnfinalbudget, true, true, false},
        {"blazarium", "mnsync", &mnsync, true, true, false},
        {"blazarium", "spork", &spork, true, true, false},
#ifdef ENABLE_WALLET
        {"blazarium", "Darksend", &Darksend, false, false, true}, /* not threadSafe because of SendMoney */
...
void StartRPCThreads()
{
    ...
    if (((mapArgs["-rpcpassword"] == "") ||
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
                                             _("To use blazariumd, or the -server option to blazarium-qt, you must set an rpcpassword in the configuration file:\n"
                                               "%s\n"
...
{
    return "> blazarium-cli " + methodname + " " + args + "\n";
}
...
std::string HelpExampleRpc(string methodname, string args)
{
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"1.0\", \"id\":\"curltest\", "
           "\"method\": \"" +
           methodname + "\", \"params\": [" + args + "] }' -H 'content-type: text/plain;' http://127.0.0.1:22789/\n";
}
