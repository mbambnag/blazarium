...
#if defined(NDEBUG)
#error "Blazarium cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Blazarium Signed Message:\n";
...
int64_t GetBlockValue(int nHeight)
{
	...
	if (nHeight == 1) return 72000 * COIN;
		
	int64_t nSubsidy;
	
	if( nHeight > 1 && nHeight <= 1500 ) {
	        nSubsidy = 1 * COIN;
	} else if( nHeight > 1500 && nHeight <= 10000 ) {
	        nSubsidy = 20 * COIN;
...
{
    RenameThread("blazarium-scriptch");
